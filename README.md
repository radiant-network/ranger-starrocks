...
| `RANGER_USERSYNC_USER` | `rangerusersync` | User allowed to download the service's policies (`policy.download.auth.users`). See [Security hardening](#security-hardening). Under the Helm chart, set via `starrocks.usersyncUser`. |
| `RANGER_TAGSYNC_USER` | `rangertagsync` | User allowed to download the service's tags (`tag.download.auth.users`). See [Security hardening](#security-hardening). Under the Helm chart, set via `starrocks.tagsyncUser`. |
